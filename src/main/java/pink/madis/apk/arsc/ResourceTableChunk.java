...
public final class ResourceTableChunk extends ChunkWithChunks {
  ...
  protected ResourceTableChunk(ByteBuffer buffer, Chunk parent) {
    super(buffer, parent);
  ...
  public void appendPackageChunk(PackageChunk packageChunk) {
    String packageName = packageChunk.getPackageName();
    if (packages.get(packageName) != null) {
      int suffix = 0;
      String originPackageName = packageName;
      do {
        packageName = originPackageName + "_" + suffix;
      } while (packages.get(packageName) != null);
      //throw new IllegalStateException("packageName:" + packageName + " PackageChunk already exist when appendPackageChunk()");
    }
    appendChunk(packageChunk);
    packages.put(packageName, packageChunk);
  }
}
