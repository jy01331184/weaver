...
public final class PackageChunk extends ChunkWithChunks {
  ...
  /** The LibraryChunk for this package, a list of package-id to package name mappings */
  private LibraryChunk library;
  ...
  private final Map<Integer, List<TypeChunk>> types = new HashMap<>();

  protected PackageChunk(ByteBuffer buffer, Chunk parent) {
    super(buffer, parent);
    id = buffer.getInt();
  ...
  protected void init(ByteBuffer buffer) {
    ...
    for (Chunk chunk : getChunks().values()) {
      if (chunk instanceof TypeChunk) {
        ...
        List<TypeChunk> typeChunks = types.get(typeChunk.getId());
        if (typeChunks == null) {
          typeChunks = new ArrayList<>();
          types.put(typeChunk.getId(), typeChunks);
        }
  ...
  public void print() {
    System.out.println("PacakgeChunk");
    System.out.println("Id:" + id);
    System.out.println("PacakgeName:" + packageName);
    System.out.println("LastPublicType:" + lastPublicType);
    System.out.println("LastPublicKey:" + lastPublicKey);
    System.out.println("TypeStringPool:" + getTypeStringPool().getStringCount());
    printStringPool(getTypeStringPool());
    System.out.println("KeyStringPool" + getKeyStringPool().getStringCount());
    printStringPool(getKeyStringPool());
  }

  private void printStringPool(StringPoolChunk stringPoolChunk) {
    for(int i = 0; i < stringPoolChunk.getStringCount(); i++) {
      System.out.println(i + ":" + stringPoolChunk.getString(i));
    }
  }
  ...
  public Map<Integer, List<TypeChunk>> getTypeChunks() {
    return types;
  }
   ...
   */
  public List<TypeChunk> getTypeChunks(int id) {
    return types.get(id);
  }
  ...
  public Map<Integer, TypeSpecChunk> getTypeSpecChunks() {
    return typeSpecs;
  }
  ...
  /** The LibraryChunk for this package, a list of package-id to package name mappings */
  public LibraryChunk getLibrary() {
    return library;
  }
